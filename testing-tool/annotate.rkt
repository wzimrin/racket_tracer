...
(make-syntax-hash (string->list "#lang racket
(+ 1 2)"))
         ...
         (let-values ([(a b)
                       (annotate-stx x
                                     (lambda (frame expanded original is-tail?)
                                       (displayln (unbox last-name))
                                       (when (not (empty? (unbox last-name)))
                                         (set-box! last-name empty))
                                       ;(displayln a)
                                       (displayln original)
                                       (displayln expanded)
                                       (displayln (hash-ref syntax-hash
